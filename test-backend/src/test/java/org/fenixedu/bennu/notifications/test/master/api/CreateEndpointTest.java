...
public class CreateEndpointTest extends AbstractAPITest {
	...
	public void success() {
		...
		DispatchedNotification notification = FenixFramework
				.getDomainObject(json.get(ID).getAsString());
		...
		assertEquals("notification user should be the same", user,
				notification.getUser());
		assertEquals("notification payload should be the same as in response",
				payloadJson, notification.getPayload().getContent());
	}
	...
	private JsonArray getUsernamesJsonArray(User... users) {
		...
		for (User user : users) {
			JsonElement usernameJsonElement = new JsonPrimitive(
					user.getUsername());
